...
const MapPicker = dynamic(() => import("@/components/map/MapPicker"), { ssr: false, loading: () => <div className="h-56 w-full rounded-2xl bg-gray-100 border-2 border-gray-200 flex items-center justify-center text-gray-400 text-sm"><Loader2 className="w-5 h-5 animate-spin mr-2" /> Loading map...</div> });
      ...
      <form onSubmit={handleSubmit} className="card p-8 space-y-6">
        ...
        <div>
          <label className="input-label flex items-center gap-1.5">
            ...
            <span className="text-xs text-gray-400 font-normal">(click map to place pin)</span>
          </label>
          <MapPicker
            lat={latitude}
            lng={longitude}
        ...
        <div>
          ...
          {images.length === 0 && !uploading && (
            <p className="text-xs text-gray-400 mt-2 flex items-center gap-1">
              <ImageIcon className="w-3.5 h-3.5" /> Photos increase enquiries by 3x. Upload at least one.
            </p>
          )}
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pt-4 border-t border-gray-100">
          <div>
            <label className="input-label mb-1">Initial Availability</label>
        ...
        <div className="flex items-center justify-end gap-3 pt-6 border-t border-gray-100">
          ...
          <button type="submit" className="btn-primary" disabled={loading || uploading || containsPhone(title) || containsPhone(description)}>
            {loading ? <><Loader2 className="w-4 h-4 animate-spin" /> Submitting...</> : "Submit Property"}
          </button>
        </div>
      </form>
    </div>
